...
test('senior-1b-accessible uses 0.9 m doors, a 1.5 m turning circle and a roll-in shower', () => {
  const t = UNIT_TEMPLATES['senior-1b-accessible'];
  const layout = layoutUnit(makeRequest(t, 0));
  for (const d of layout.doors) {
    if (d.type === 'closet' || d.type === 'service' || d.operation === 'NOTDEFINED' || d.type === 'balcony') continue;
...
test('coliving-cluster puts six en-suite bedrooms off a corridor with a shared hearth', () => {
...
test('dual-key gives the studio its own bath, kitchenette and door off the vestibule', () => {
...
test('corner-2b2b gives the living room light on two sides (ARC-26)', () => {
...
test('townhouse-3s puts the garage at ground, living on the first floor and bedrooms above', () => {
...
test('regional vocabulary follows the requested region', () => {
...
test('layout is deterministic for the same request', () => {
...
test('degrades gracefully in a rect that is far too small', () => {
  const t = UNIT_TEMPLATES['3b2b'];
...
test('every access side and a rotated rect produce the same plan quality', () => {
  const t = UNIT_TEMPLATES['2b2b'];
...
test('ARC-36 through flat: 2b2b entered from the landing, living room on the sunny façade', () => {
...
test('ARC-36 through flat: flipping the exposures moves the living room to the other façade', () => {
  const req = makeSideEntryRequest({
    templateId: '2b2b', rect: { x: 10, y: 4, w: 8, h: 15.4 }, access: 'right',
...
test('ARC-36 through flat: an end-of-bar unit with three exterior sides keeps every room lit', () => {
  const req = makeSideEntryRequest({
...
test('ARC-36 through flat: a frontage too short for every habitable room says so once', () => {
  const req = makeSideEntryRequest({
...
test('ARC-16 glazing hits the window-to-wall target on each exterior wall', () => {
...
test('ARC-16 counts a balcony door as glazing and still reaches the target', () => {
  const req = makeSideEntryRequest({
...
test('ARC-16 reports the shortfall when a garage or a store blocks the façade', () => {
...
test('all 20 templates lay out in under 50 ms', () => {
  const run = (): number => {
    let n = 0;
    for (const id of UNIT_TEMPLATE_IDS) {
      const t = UNIT_TEMPLATES[id];
