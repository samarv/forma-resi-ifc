...
export interface WallParams extends ElementAttributes {
  ...
  /**
   * `IfcWallTypeEnum` token, IFC4/IFC4X3 only (default `STANDARD`):
   * `MOVABLE` | `PARAPET` | `PARTITIONING` | `PLUMBINGWALL` | `SHEAR` |
   * `SOLIDWALL` | `STANDARD` | `POLYGONAL` | `ELEMENTEDWALL` | `USERDEFINED` |
   * `NOTDEFINED`.
   */
...
export interface SlabParams extends ElementAttributes {
  ...
  Openings?: RectangularOpening[];
  /**
   * IfcSlabTypeEnum token without dots. Defaults to FLOOR.
   * (forma-resi-ifc addition — see NOTICE.md)
   */
  PredefinedType?: 'FLOOR' | 'ROOF' | 'LANDING' | 'BASESLAB' | 'USERDEFINED' | 'NOTDEFINED';
}
...
export interface RoofParams extends ElementAttributes {
  ...
  Slope?: number;
  /**
   * IfcRoofTypeEnum token without dots (FLAT_ROOF, SHED_ROOF, GABLE_ROOF, HIP_ROOF, …).
   * Defaults to FLAT_ROOF. (forma-resi-ifc addition — see NOTICE.md)
   */
  PredefinedType?: string;
}
...
export interface GableRoofParams extends ElementAttributes {
  ...
  Overhang?: number;
  /**
   * IfcRoofTypeEnum token without dots. Defaults to GABLE_ROOF (the previously
   * hardcoded value); IFC2X3, whose enum has no USERDEFINED, falls back to
   * GABLE_ROOF for that token. (forma-resi-ifc addition — see NOTICE.md)
   */
  PredefinedType?: 'FLAT_ROOF' | 'SHED_ROOF' | 'GABLE_ROOF' | 'HIP_ROOF' | 'HIPPED_GABLE_ROOF'
...
export interface RailingParams extends ElementAttributes {
  ...
  Width?: number;
  /**
   * IfcRailingTypeEnum token without dots. Defaults to HANDRAIL.
   * (forma-resi-ifc addition — see NOTICE.md)
   */
  PredefinedType?: 'HANDRAIL' | 'GUARDRAIL' | 'BALUSTRADE' | 'USERDEFINED' | 'NOTDEFINED';
}
...
export interface PileParams extends ElementAttributes {
  ...
  /**
   * IfcPileTypeEnum token without dots (BORED, DRIVEN, JETGROUTING, COHESION,
   * FRICTION, SUPPORT, USERDEFINED, NOTDEFINED). Defaults to DRIVEN.
   * (forma-resi-ifc addition — see NOTICE.md)
   */
  PredefinedType?: string;
}
...
export interface SpaceParams extends ElementAttributes {
  ...
  Profile?: Point2D[];
  /**
   * IFC4 `IfcSpaceTypeEnum` token without dots: SPACE | PARKING | GFA | INTERNAL |
   * EXTERNAL | USERDEFINED | NOTDEFINED. Defaults to INTERNAL. Under IFC2X3 the
   * attribute is `InteriorOrExteriorSpace`, so anything other than EXTERNAL /
   * NOTDEFINED / USERDEFINED is written as INTERNAL.
   * (forma-resi-ifc addition — see NOTICE.md)
   */
  PredefinedType?: 'SPACE' | 'PARKING' | 'GFA' | 'INTERNAL' | 'EXTERNAL' | 'USERDEFINED' | 'NOTDEFINED';
}
...
export interface ProjectParams {
  ...
  /**
   * Name/Description for the single IfcSite of the preamble (defaults to 'Site').
   * (forma-resi-ifc addition — see NOTICE.md)
   */
  Site?: SiteParams;
  /**
   * Name/Description for the single IfcBuilding of the preamble (defaults to 'Building').
   * (forma-resi-ifc addition — see NOTICE.md)
   */
  ...
  Organization?: string;
  /**
   * STEP `FILE_NAME` first field — the logical file name. Defaults to
   * 'created.ifc'. (forma-resi-ifc addition — see NOTICE.md)
   */
  FileName?: string;
  /**
   * STEP `FILE_DESCRIPTION` first field. IFC uses it to declare the model view
   * definition, e.g. 'ViewDefinition [CoordinationView]'. Defaults to
   * 'Created by ifc-lite'. (forma-resi-ifc addition — see NOTICE.md)
   */
...
/**
 * Grouping options for `addIfcSystem` (forma-resi-ifc addition — see NOTICE.md).
 *
 * Under IFC2X3 there is no IfcDistributionSystem and IfcSystem carries no
 * PredefinedType/LongName, so `IfcType`/`PredefinedType` are ignored there.
 */
export interface SystemParams {
  /** Entity to emit. Defaults to IFCDISTRIBUTIONSYSTEM under IFC4/IFC4X3. */
  IfcType?: 'IFCSYSTEM' | 'IFCDISTRIBUTIONSYSTEM';
  /** IfcDistributionSystemEnum token without dots (e.g. 'DOMESTICCOLDWATER'). */
  PredefinedType?: string;
  ObjectType?: string;
  Description?: string;
  LongName?: string;
  /** Emit IfcRelServicesBuildings linking the system to the building. Default true. */
  ServesBuilding?: boolean;
}

/** Grouping options for `addIfcZone` (forma-resi-ifc addition — see NOTICE.md). */
